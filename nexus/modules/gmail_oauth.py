"""
Gmail OAuth Service
Manages OAuth2 authentication and token storage for Gmail API access.
"""
import os
import json
import logging
...
logger = logging.getLogger("nexus.gmail_oauth")
...
class GmailOAuthService:
    """
    Manages Gmail OAuth2 authentication flow and token storage.
    """
    ...
    def __init__(self):
        ...
        if not self.client_id or not self.client_secret:
            logger.warning("Gmail OAuth credentials not configured (GMAIL_CLIENT_ID, GMAIL_CLIENT_SECRET)")
    ...
    async def store_credentials(self, user_id: str, credentials: Credentials, email: Optional[str] = None) -> bool:
        ...
        try:
            ...
            if not email:
                try:
                    # Try to get email from Gmail API profile
            ...
            logger.info(f"Stored Gmail OAuth credentials for user {user_id}, email {email}")
            return True
        except Exception as e:
            logger.error(f"Failed to store Gmail OAuth credentials: {e}")
            return False
    ...
    async def get_credentials(self, user_id: str, email: Optional[str] = None) -> Optional[Credentials]:
        ...
        Args:
            user_id: User identifier
            email: Optional email filter (if user has multiple Gmail accounts)
    ...
    async def list_user_emails(self, user_id: str) -> list:
        """
        List all Gmail accounts connected for user.
