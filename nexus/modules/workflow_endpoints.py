...
async def start_shaping(req: StartShapingRequest):
    ...
    """
    logger.info(f"Starting Shaping Session for user={req.user_id}, query='{req.query}'")
    ...
    try:
        ...
        logger.info(f"Diagnosis complete (Mock/Heuristic). Found {len(candidates)} candidates.")
...
async def shaping_chat(session_id: int, req: ChatRequest):
    ...
    """
    logger.info(f"Chat Request on Session {session_id}: '{req.message}'")
    ...
    logger.debug(f"Generated Prompt: {mock_prompt}")
    ...
    logger.info(f"LLM executed. Trace ID: {trace_id}")
