...
Manages OAuth2 authentication and token storage for Google Calendar API access.
Similar to gmail_oauth but for Calendar API.
"""
import os
import json
import logging
...
logger = logging.getLogger("nexus.calendar_oauth")
...
CLIENT_ID = os.getenv("GMAIL_CLIENT_ID")  # Can reuse Gmail OAuth credentials
...
class CalendarOAuthService:
    """
    Manages Google Calendar OAuth2 authentication flow and token storage.
    Similar to GmailOAuthService but for Calendar API.
    """
    ...
    def __init__(self):
        ...
        if not self.client_id or not self.client_secret:
            logger.warning("Calendar OAuth credentials not configured (GMAIL_CLIENT_ID, GMAIL_CLIENT_SECRET)")
    ...
    async def store_credentials(self, user_id: str, credentials: Credentials, email: Optional[str] = None) -> bool:
        ...
        try:
            ...
            if not email:
                try:
                    # Try to get email from Calendar API or userinfo
            ...
            # Store in database (reuse gmail_oauth_tokens table or create separate calendar table)
            # For now, using same table with a scope check
            query = """
            ...
            logger.info(f"Stored Calendar OAuth credentials for user {user_id}, email {email}")
            return True
        except Exception as e:
            logger.error(f"Failed to store Calendar OAuth credentials: {e}")
            return False
    ...
    async def get_credentials(self, user_id: str, email: Optional[str] = None) -> Optional[Credentials]:
        ...
        try:
            if email:
                query = """
                    SELECT encrypted_token, encrypted_refresh_token, token_uri, client_id, client_secret, scopes
            ...
            creds_dict = {
                ...
                'client_secret': row['client_secret'] or self.client_secret,
            ...
            logger.error(f"Failed to retrieve Calendar OAuth credentials: {e}")
            return None
    ...
    async def list_user_calendars(self, user_id: str) -> list:
        """
        List all Google Calendar accounts connected for user.
        ...
        try:
            query = """
                ...
                FROM gmail_oauth_tokens
                WHERE user_id = :user_id
                AND (scopes::text LIKE '%calendar%' OR scopes::text LIKE '%Calendar%')
                ORDER BY updated_at DESC
            """
            rows = await database.fetch_all(query, {'user_id': user_id})
            return [{'email': row['email'], 'updated_at': row['updated_at']} for row in rows]
        except Exception as e:
