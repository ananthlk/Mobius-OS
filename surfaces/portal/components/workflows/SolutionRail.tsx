...
export default function SolutionRail({ 
    ...
    const [localSteps, setLocalSteps] = useState<any[]>(draftPlan?.steps || []);
    const [expandedCards, setExpandedCards] = useState<Set<string>>(new Set());

    // Sync local steps with draftPlan prop
    if (draftPlan?.steps && JSON.stringify(localSteps) !== JSON.stringify(draftPlan.steps)) {
        setLocalSteps(draftPlan.steps);
    }
    ...
    const handleDrop = (e: React.DragEvent, targetIndex: number) => {
        ...
        const draggedIndex = localSteps.findIndex(s => s.id === draggedStepId);
    ...
    const handleSaveEdit = async (stepId: string, updatedStep: any) => {
        const newSteps = localSteps.map(s => s.id === stepId ? updatedStep : s);
        setLocalSteps(newSteps);
    ...
    return (
        <div className="w-full bg-white border-r-2 border-gray-300 flex flex-col h-full z-20">
            {/* TOP PANEL: MATCHING WORKFLOWS */}
            <div className="flex-[0_0_30%] flex flex-col min-h-0 border-b-2 border-gray-300 bg-white overflow-hidden">
                ...
                <div className="flex-1 overflow-y-auto px-4 py-3 space-y-1.5 custom-scrollbar min-h-0">
                    {solutions.map((sol) => {
                        ...
                        const hasDescription = sol.description && sol.description.length > 0;
                        ...
                        return (
                            ...
                            >
                                ...
                                {isExpanded && hasDescription && (
                                    <div className="px-2.5 pb-2.5 pt-0 border-t border-gray-100 mt-1">
            ...
            {/* BOTTOM PANEL: LIVE DRAFT */}
            <div className="flex-[1_1_70%] flex flex-col min-h-0 bg-white border-t-2 border-gray-300">
                <div className="px-4 py-4 border-b-2 border-gray-300 bg-gradient-to-b from-yellow-50/80 to-white flex-shrink-0 shadow-sm">
                    ...
                    {draftPlan?.problem_statement && (
                        <div className="mb-3 p-3 bg-amber-50 border border-amber-200 rounded-lg">
                            <p className="text-[10px] text-amber-800 font-medium mb-1.5 uppercase tracking-wide">Problem Statement</p>
                            <p className="text-sm text-gray-900 leading-relaxed">{draftPlan.problem_statement}</p>
                        </div>
                    )}
                    {draftPlan?.name && (
                                ...
                                return (
                                    ...
                                    >
                                        <div className="flex flex-col items-center flex-shrink-0 pt-0.5">
                                            <Tooltip content={`Step ${idx + 1} - Drag to reorder steps in the workflow`}>
                                            ...
                                            {idx < (localSteps.length - 1) && <div className="w-px flex-1 bg-gray-200 my-1"></div>}
                                        </div>
                                        <div className="flex-1 min-w-0">
                                            {isEditing ? (
                                                ...
                                                <div className="bg-white p-4 rounded-lg border border-gray-200 hover:border-gray-300 transition-colors relative group/card">
                                                    {/* Action buttons */}
                                                    ...
                                                    {/* Header with tool name and indicators */}
                                                    ...
                                                    {/* Description */}
                                                    <div className="text-sm text-gray-700 leading-relaxed mb-3">{step.description}</div>
                                                    ...
                                                    {step.execution_conditions && step.execution_conditions.length > 0 && (
                                                        <div className="mt-3 pt-3 border-t border-gray-100">
                                                            ...
                                                            <div className="flex flex-wrap gap-1.5">
                                                                {step.execution_conditions.map((condition: any, idx: number) => {
