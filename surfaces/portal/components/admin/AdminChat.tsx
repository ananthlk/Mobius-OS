...
export default function AdminChat({ 
    ...
    const sendMessage = async (e?: React.FormEvent | React.KeyboardEvent) => {
        ...
        try {
            ...
            const response = await fetch(`${apiUrl}/api/portal/chat`, {
                ...
                body: JSON.stringify({
                    user_id: session?.user?.email || "admin_user",
                    messages: newHistory,
                    stream: false,
                }),
            });
    ...
    return (
        <div className="h-full flex flex-col">
            ...
            <div className="flex-1 overflow-y-auto p-4 md:p-6 space-y-8">
                {messages.length === 0 && (
                    <div className="h-full flex flex-col items-center justify-center -mt-10">
                        ...
                        <h2 className="text-2xl font-normal mb-2">{emptyStateTitle}</h2>
                        <p className="text-[var(--text-secondary)] mb-8">{emptyStateDescription}</p>
                    </div>
                )}
            ...
            {/* Input Area */}
            <div className="p-4 md:p-6 border-t border-[var(--border-subtle)]">
                ...
                <div className="text-center mt-3 text-xs text-[var(--text-muted)]">
                    Möbius can make mistakes. Verify critical information.
                </div>
            </div>
        </div>
    );
